  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        {footer && (
          <div className="mt-6 pt-4 border-t border-zinc-800">{footer}</div>
        )}
      </motion.div>
    </div>
  );
...
export default function ConatctManagement() {
  ...
  const [newLead, setNewLead] = useState({
    name: "", company: "", status: "New", last: "Aug 20",
  });
  ...
  return (
      ...
      <div className="min-h-screen bg-black text-white mt-40">
        {/* Navbar */}
      <header className="fixed top-20 left-0 right-0 z-50 bg-black/40 backdrop-blur border-b border-zinc-800">
        <div className="relative flex items-center justify-center px-6 py-3">
          {/* Brand (kept left corner) */}
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm transition ${
                  activeTab === item.label
                    ? "bg-zinc-900/70 border-zinc-800 text-yellow-400"
                    : "hover:bg-zinc-900/70 border-transparent hover:border-zinc-800 text-zinc-300"
                }`}
              >
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Section
                  ...
                  right={
                    <button onClick={() => setTaskModal(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-zinc-800 hover:bg-zinc-900 text-sm">
                ...
                >
                  <ul className="space-y-3">
                    {tasks.map((t) => (
                      <li key={t.id} className="flex items-center justify-between bg-zinc-900/40 border border-zinc-800 rounded-xl px-3 py-2">
        ...
        <Modal
          ...
          footer={
            <div className="flex justify-end gap-2">
              <button onClick={() => setLeadModal(false)} className="px-3 py-2 rounded-lg border border-zinc-800 hover:bg-zinc-900 text-sm">Cancel</button>
        ...
        >
          <form onSubmit={addLead} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="text-sm space-y-1">
              ...
              <input value={newLead.name} onChange={(e) => setNewLead((v) => ({ ...v, name: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 outline-none" required />
            </label>
            <label className="text-sm space-y-1">
              <span className="text-zinc-400">Company</span>
              <input value={newLead.company} onChange={(e) => setNewLead((v) => ({ ...v, company: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 outline-none" required />
            </label>
          </form>
        </Modal>
        ...
        <Modal
          ...
          footer={
            <div className="flex justify-end gap-2">
              <button onClick={() => setTaskModal(false)} className="px-3 py-2 rounded-lg border border-zinc-800 hover:bg-zinc-900 text-sm">Cancel</button>
        ...
        >
          <form onSubmit={addTask} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="text-sm space-y-1">
              ...
              <input value={newTask.title} onChange={(e) => setNewTask((v) => ({ ...v, title: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 outline-none" required />
            </label>
          </form>
        </Modal>
      </div>
