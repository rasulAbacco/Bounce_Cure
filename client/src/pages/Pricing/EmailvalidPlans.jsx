...
import { User, Mail, Headphones, Check, X, Shield, ArrowLeft, ArrowRight } from "lucide-react";

// Email Verification Plans Component
const EmailValidationPlans = ({ selectedCurrency: propCurrency, currencyRates, onClose }) => {
  ...
  const handleBuyVerification = (tier) => {
    ...
    console.log('Email Verification Plan:', verificationPlan);
    alert(`Selected: ${verificationPlan.planName}\nPrice: ${rates[selectedCurrency]?.symbol}${convertedPrice.toFixed(2)}`);
  };
...
const PricingDash = () => {
  ...
  const handleBuyNow = (plan, contacts) => {
    const hasPurchasedBefore = false;
    ...
    alert(`Selected Plan: ${plan.name}\nContacts: ${contacts}\nEmail Sends: ${emailSendLimit.toLocaleString()}\nEmail Validations: ${emailValidationLimit.toLocaleString()}\nPrice: ${currencySymbols[selectedCurrency]}${convertedBasePrice.toFixed(2)}`);
  };
            ...
            <div className="inline-flex rounded-md shadow-sm overflow-hidden border border-gray-700">
              {["monthly", "quarterly", "yearly"].map((type) => (
                <button
                  ...
                  } ${type === "monthly" ? "rounded-l-lg" : ""} ${type === "yearly" ? "rounded-r-lg" : ""}`}
                  ...
                  <ul className="text-sm text-gray-300 mb-4 space-y-2 text-left">
                    ...
                    <li className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          ...
                          {plan.name === "Free"
                            ? "50"
                            : `${calculateEmailSends(plan, selectedContacts).toLocaleString()} `}
                        </span>
                      </div>
                    ...
                    {plan.emailValidationPerContact > 0 && (
                      <li className="flex items-center gap-2">
                        ...
                        {`${calculateEmailValidations(plan, selectedContacts).toLocaleString()}  `}
                      </li>
                    )}
