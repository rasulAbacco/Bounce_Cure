    ...
    useEffect(() => {
        ...
        const handleCallback = async () => {
            ...
            if (!code || !clientId || !clientSecret) {
                alert("Missing required parameters from Outlook OAuth. Ensure client ID/secret and redirect URI match.");
                navigate("/");
                return;
            }
            ...
            try {
                ...
                const res = await api.get("/auth/outlook/callback", {
                    params: {
                        code,
                        clientId,
                        clientSecret,
                ...
                console.error("OAuth callback failed:", err.response?.data || err.message || err);
                alert("❌ Failed to complete Outlook OAuth: " + (err.response?.data?.error_description || err.message));
                navigate("/");
            }
        };
