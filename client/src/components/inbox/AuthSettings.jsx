        ...
        <div>
          ...
          <div className="mt-2 p-3 bg-blue-900 bg-opacity-30 border border-blue-700 rounded">
            <p className="text-sm text-blue-200">
              {form.provider === "zoho" && (
                <>
                  <strong>Zoho:</strong> Go to Settings → Security → App Passwords to generate one.
                </>
              )}
              {form.provider === "gmail" && (
                <>
                  <strong>Gmail:</strong> Go to Google Account → Security → 2-Step Verification → App passwords.
                </>
              )}
              {form.provider === "outlook" && (
                <>
                  <strong>Outlook:</strong> Go to Account Settings → Security → App passwords.
                </>
              )}
              {form.provider === "rediff" && (
                <>
                  <strong>Rediff Mail Pro:</strong> Go to Mail Settings → Security → Generate App Password.
                </>
              )}
              {form.provider === "amazon" && (
                <>
                  <strong>Amazon WorkMail:</strong> Go to Amazon WorkMail settings → 
                  IMAP/POP settings → Enable IMAP access. Generate an app password in 
                  your AWS account under Security credentials.
                </>
              )}
              {!["zoho", "gmail", "outlook", "rediff", "amazon"].includes(form.provider) && (
                <>Generate an app-specific password from your email provider's security settings.</>
              )}
            </p>
          </div>
        </div>
        ...
        <>
          ...
          {form.provider === "zoho" ? (
            <div className="space-y-4">
              ...
              <div>
                <label className="block mb-2 text-sm">OAuth Client Secret</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block mb-2 text-sm">OAuth Refresh Token</label>
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Click "Get Token" to authorize with Zoho. Copy the refresh token from the page that opens.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Click "Get Token" to authorize with Google. Copy the refresh token from the response.
                </p>
              </div>
            </div>
          ) : form.provider === "yahoo" ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Click "Get Token" to authorize with Yahoo. Copy the refresh token from the response.
                </p>
              </div>
            </div>
          ) : form.provider === "rediff" ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Click "Get Token" to authorize with Rediff. Copy the refresh token from the response.
                </p>
              </div>
            </div>
          ) : (
