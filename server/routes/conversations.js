  ...
  try {
    const emails = await prisma.email.findMany({
      where: {
        isReply: false,
        folder: "INBOX",
        account: { userId: req.user.id }, // ✅ filter by logged-in user
      },
      orderBy: { date: "desc" },
      take: 100,
      ...
      emails.map(async (e) => {
        ...
        const replyCount = await prisma.email.count({
          where: {
            ...
            id: { not: e.id },
            account: { userId: req.user.id }, // ✅ restrict replies too
          },
        });
  ...
  try {
    ...
    const threadEmails = await prisma.email.findMany({
      where: {
        ...
        account: { userId: req.user.id }, // ✅ restrict
      },
      orderBy: { date: "asc" },
      include: { account: true },
    });
...
router.post("/:id/reply", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res.json({ ...message, status: "sent" });
    } catch (emailError) {
      await prisma.email.update({
        where: { id: message.id },
        data: { status: "failed" },
