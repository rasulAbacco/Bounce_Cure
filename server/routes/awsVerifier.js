...
export default class AdvancedVerifier {
    ...
    async verify(email, { smtpCheck = false, isBulk = false, fast = false } = {}) {
        ...
        if (!syntax_valid) {
            this.stats.invalid++;
            return { ...resultTemplate, syntax_valid, error: "Invalid syntax" };
        }
        ...
        if (!domain_valid) {
            this.stats.invalid++;
            return { ...resultTemplate, syntax_valid, domain_valid, error: "No MX records found" };
        }
        ...
        try {
            ...
            if (resp.ok) {
                ...
                const status = providerResp?.validationResult?.status;

                if (status === "valid") mailbox_exists = true;
                else if (status === "invalid") mailbox_exists = false;
        ...
        // 5. SMTP Fallback (if enabled and needed)
        if (!fast && smtpCheck && mailbox_exists === null) {
            try {
                ...
                mailbox_exists = rcpt.code === 250 ? true : rcpt.code === 550 ? false : null;
        ...
        // 7. Final result
        const finalResult = this.computeFinalStatusAndScore({
            email,
            syntax_valid,
            domain_valid,
            mailbox_exists,
    ...
    computeFinalStatusAndScore(r) {
        ...
        if (!r.syntax_valid || !r.domain_valid) {
            ...
            // unknown → treat as invalid (strict)
            status = "invalid";
            score = 0;
        }
